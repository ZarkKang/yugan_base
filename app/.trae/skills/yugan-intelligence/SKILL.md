...
## 开发日志机制 (`logs/devlog.md`)
项目日志目录 `logs/` 下维护 `devlog.md`，作为所有重要变更的时序记录。每次对话结束时，如果发生了下列**重大变更**，你必须自动更新该文件：
- 新增/删除子系统或核心服务
- 数据库表结构变更（新增表、字段、索引）
- API 接口新增、破坏性变更或废弃
- 关键 Bug 修复（影响核心流程）
...
- 无人机端接口文档位于 `doc/drone/无人机端开发规范.md`，该文档是基站与无人机之间的**唯一接口真相源**。
- 任何基站接口变更（新增端点、修改字段、调整路径）必须同步更新 `doc/drone/无人机端开发规范.md`。
...
### 无人机端开发文档 (`doc/drone/无人机端开发规范.md`)
...
**使用规则**：
- 每次基站开发任务开始时，必须先读取 `doc/drone/无人机端开发规范.md` 检查是否需要同步更新无人机端。
- 无人机端开发智能体可直接以该文档为需求规格说明书进行开发。
- 文档中的接口定义、数据格式、测试标准对无人机端具有约束力。

### 文档版本控制机制

1. **版本号规则**：`doc/drone/无人机端开发规范.md` 采用语义化版本 `v<主版本>.<次版本>`。
...
- RFID 驆动：`src/hardware/rfid_reader.py`（PRE 模块, V2.2）
- RFID 协议文档：`doc/PRE系列一体化模块多接口资料/`
- 异常处理：`src/core/exceptions.py`
- 项目记忆：`project_memory.md`（启动时必读）
- 开发日志：`logs/devlog.md`（变更记录）
- 网关路由配置：`api-gateway/main.py`
- **无人机端开发规范**：`doc/drone/无人机端开发规范.md`（基站-无人机接口契约，开发前必读）
- **复盘报告**：`logs/review/` 目录下的状态分析报告

## 交互原则
- 在开发前如果发现资料冲突或缺失（如 SKU 缺少 epc 字段），立即提问，**禁止臆测**。
- 输出代码时只给必要的文件和注释，避免冗余解释。
- 任何影响系统全局的变更（如新增库表、添加依赖、修改网关路由），必须征求确认。
- **每次新对话开始时，主动重新读取 `project_memory.md`、`logs/devlog.md` 以及本 Skill 文件**，以恢复完整的项目上下文。